...
public class CardResponse {
	...
	public static HttpResponseMessage generate(HttpRequestMessage<Optional<String>> s, String textResponse, Card card) {
		...
		if (card.getSubtitle().isPresent()) {
			jsonCard.put("subtitle", card.getSubtitle().get());
		}
	...
	public static HttpResponseMessage generateWithButton(HttpRequestMessage<Optional<String>> s, String textResponse, Card card) {
		// return the response based on the rows created
		...
		if (card.getImgUrl().isPresent() ) {
			jsonCard.put("buttons", new JSONArray().put(new JSONObject().put("title", card.getButton_title().get()).put("openUrlAction",
					new JSONObject().put("url", card.getButton_url().get()))));
		}
